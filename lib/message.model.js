...
const MessageSchema = new Schema({
     ...
     * @instance
     */
    sender: { // TODO: use contact schema
      type: String,
      trim: true,
      required: true,
      index: true,
...
exports.SMS = function SMS(payload) {
  const copyOfpayload = mergeObjects({ transport: DEFAULT_SMS_TRANSPORT_NAME },
    payload, { type: TYPE_SMS }
  );
...
exports.Push = function Push(payload) {
  const _payload = mergeObjects({ transport: DEFAULT_PUSH_TRANSPORT_NAME },
    payload, { type: TYPE_PUSH }
  );
  return new Message(_payload);
};