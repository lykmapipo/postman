...
const { hashOf } = require('@lykmapipo/common');
const { getString, getBoolean } = require('@lykmapipo/env');
const { model, Schema, Mixed } = require('@lykmapipo/mongoose-common');
...
const STATES = [
  ...
  STATE_QUEUED, STATE_DELIVERED, STATE_FAILED
];
    ...
    waterfall([
      ...
      function send(next) {
        ...
        transport.send(this, function (error, result) {
          ...
          //prepare result
          let _result = _.merge({}, { success: true }, result);
 ...
 * 
 */
MessageSchema.methods.queue = function queue() {
 ...
 * @private
 */
module.exports = exports = model(MODEL_NAME, MessageSchema);