...
let MessageSchema = new Schema({
  ...
  /**
   * @name bulk
   * @description unique identifier used to track message which have been
   * send together.
   * @type {Object}
   * @since 0.1.0
   * @version 1.0.0
...
MessageSchema.methods.send = function send(done) {
  ...
  /* @todo return message always */

  //debug(fake) message send
  if (DEBUG) {
    ...
    return this.put(done); /* @todo save */
...
MessageSchema.methods.queue = function queue( /*done*/ ) {
  ...
  /* @todo job should save & queue */
  /* @todo return job always */
  ...
  this.save(function (error, message) {
    ...
    //notify error
    if (error) {
      worker.queue.emit('message:queue:error', error);
    }
    ...
    //for actul send
      ...
      worker.queue.emit('message:queue:success', message);
    }
    ...
    //push transport are notified in their worker to send the message
      ...
      const job = this.runInBackground(jobDetails);
 ...
 * @param {Object} [criteria] valid mongoose query criteria
 * @type {Function}
 * @events message:requeue:error, message:requeue:success, message:queue:error, 
...
MessageSchema.statics.requeue = function (criteria) {
  ...
  Message.unsent(_criteria, function (error, unsents) {
    ...
    //there is message queue
      ...
      //fire requeue error
      if (error) {
        worker.queue.emit('message:requeue:error', error);
      }
      ...
      //re-queue all unsent message(s)
        ...
        worker.queue.emit('message:requeue:success', unsents);
